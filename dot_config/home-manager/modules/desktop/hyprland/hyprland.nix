    ...
    wayland.windowManager.hyprland = {
      ...
      settings = {
        ...
        animation = [
          ...
          { leaf = "windows"; enabled = true; speed = 4.5; bezier = "easeOutQuint"; }
          { leaf = "windowsIn"; enabled = true; speed = 4; bezier = "easeOutQuint"; style = "popin 80%"; }
          { leaf = "windowsOut"; enabled = true; speed = 1.5; bezier = "almostLinear"; style = "popin 80%"; }
          { leaf = "fade"; enabled = true; speed = 3; bezier = "quick"; }
        ...
        bind =
          [
            ...
            (mkBindOpts ''"XF86AudioRaiseVolume"'' ''hl.dsp.exec_cmd("${pkgs.wireplumber}/bin/wpctl set-volume -l 1 @DEFAULT_AUDIO_SINK@ 5%+")'' { repeating = true; locked = true; })
            (mkBindOpts ''"XF86AudioLowerVolume"'' ''hl.dsp.exec_cmd("${pkgs.wireplumber}/bin/wpctl set-volume @DEFAULT_AUDIO_SINK@ 5%-")'' { repeating = true; locked = true; })

            # Brightness (repeat + locked)
            (mkBindOpts ''"XF86MonBrightnessUp"'' ''hl.dsp.exec_cmd("${pkgs.brightnessctl}/bin/brightnessctl -e4 -n2 set 5%+")'' { repeating = true; locked = true; })
            (mkBindOpts ''"XF86MonBrightnessDown"'' ''hl.dsp.exec_cmd("${pkgs.brightnessctl}/bin/brightnessctl -e4 -n2 set 5%-")'' { repeating = true; locked = true; })

            # Mute (locked)
            (mkBindOpts ''"XF86AudioMute"'' ''hl.dsp.exec_cmd("${pkgs.wireplumber}/bin/wpctl set-mute @DEFAULT_AUDIO_SINK@ toggle")'' { locked = true; })
            (mkBindOpts ''"XF86AudioMicMute"'' ''hl.dsp.exec_cmd("${pkgs.wireplumber}/bin/wpctl set-mute @DEFAULT_AUDIO_SOURCE@ toggle")'' { locked = true; })

            # Playback (locked)
            (mkBindOpts ''"XF86AudioPlay"'' ''hl.dsp.exec_cmd("${pkgs.playerctl}/bin/playerctl play-pause")'' { locked = true; })
            (mkBindOpts ''"XF86AudioPause"'' ''hl.dsp.exec_cmd("${pkgs.playerctl}/bin/playerctl play-pause")'' { locked = true; })
            (mkBindOpts ''"XF86AudioNext"'' ''hl.dsp.exec_cmd("${pkgs.playerctl}/bin/playerctl next")'' { locked = true; })
            (mkBindOpts ''"XF86AudioPrev"'' ''hl.dsp.exec_cmd("${pkgs.playerctl}/bin/playerctl previous")'' { locked = true; })
          ]
          ++ workspaceBinds;
      };
    };
